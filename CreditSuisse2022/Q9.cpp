...
int horiRec(int x1, int x2, int y1, int y2, ez& memo) {
    ...
    int y = y2 - y1 + 1, x = x2 - x1 + 1, sum = 0, min = INT32_MAX;
    if (x > y) return vertRec(x1, x2, y1, y2, memo);
    if (x == y) return square(x, x1, y1, memo);
    if (x == 1) {
        for (int i = y1; i <= y2; i++) sum += memo[x1][x1][i][i];
        memo[x1][x2][y1][y2] = sum;
        return sum;
    }
    for (int i = 0; i <= y-x; i++) {
        sum += square(x, x1, y1 + i, memo);
        if (i != 0) sum += horiRec(x1, x2, y1, y1 + i - 1, memo);
        if (i != y - x) sum += horiRec(x1, x2, y1 + i + x, y2, memo);
        min = std::min(min, sum);
        if (min == 0) break;
    }
...
int vertRec(int x1, int x2, int y1, int y2, ez& memo) {
    ...
    if (x == y) return square(x, x1, y1, memo);
    if (y == 1) {
        for (int i = x1; i <= x2; i++) sum += memo[i][i][y1][y1];
        memo[x1][x2][y1][y2] = sum;
        return sum;
    }
    for (int i = 0; i <= x-y; i++) {
        sum += square(y, x1 + i, y1, memo);
        if (i != 0) sum += vertRec(x1, x1 + i - 1, y1, y2, memo);
        if (i != x-y) sum += vertRec(x1 + i + y, x2, y1, y2, memo);
        min = std::min(min, sum);
        if (min == 0) break;
    }
    memo[x1][x2][y1][y2] = min;
...
int square(int l, int x, int y, ez& memo) {
    ...
    int min = INT32_MAX;
    for (int i = 1; i < l; i++){
        min = std::min(min, square(i, x, y, memo)
            + square(l - i, x + i, y + i, memo)
            + vertRec(x + i, x + l - 1, y, y + i - 1, memo)
            + horiRec(x, x + i - 1, y + i, y + l - 1, memo));
        if (min == 0) break;
        min = std::min(min, square(i, x + l - i, y, memo)
            + square(l - i, x, y + i, memo)
            + vertRec(x, x + l - 1 - i, y, y + i - 1, memo)
