...
vector<bool> scc_has_incoming_outside_edge, scc_has_outgoing_outside_edge;
int T, n, m, a, b, num_scc;
...
int main() {
    ...
    while (T--) {
        ...
        scc_has_outgoing_outside_edge.assign(num_scc, false);
        for (int u = 0; u < n; u++) {
            for (int v : AL[u]) {
                if (scc_index[u] != scc_index[v]) {
                    scc_has_incoming_outside_edge[scc_index[v]] = true;
                    scc_has_outgoing_outside_edge[scc_index[u]] = true;
                }
            }
        }
        int no_outgoing = 0, no_incoming = 0;
        for (int i = 0; i < num_scc; i++) {
            if (!scc_has_incoming_outside_edge[i]) no_incoming++;
            if (!scc_has_outgoing_outside_edge[i]) no_outgoing++;
        }
        if (num_scc == 1) cout << 0 << '\n';
        else cout << max(no_outgoing, no_incoming) << '\n';
    }
}